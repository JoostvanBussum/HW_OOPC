...
public:
   ...
   /// print 2 values
   /// /details
   /// print out the counter and denominator as a hexadecimal value
   void operator<<(rational lhs){
       std::cout << std::hex << lhs.counter << "\n";
       std::cout << std::hex << lhs.denominator << "\n";       
   }
   ...
   bool operator==( const rational & rhs ) const {
      rational tmpRhs(rhs.counter, rhs.denominator);
      rational tmpLhs(counter, denominator);
      tmpRhs.reduce();
      tmpLhs.reduce();
      return ( tmpLhs.counter == tmpRhs.counter ) && ( tmpLhs.denominator == tmpRhs.denominator ) ;
   }
   ...
   rational operator*( const rational & rhs ) const {
       rational tmpRhs(counter*rhs.counter, denominator*rhs.denominator);
       tmpRhs.reduce();
      return rational(tmpRhs);
   }
   ...
   /// multiply a rational by a rational 
   /// \details
   /// This operator*= multiplies a rational with a rational;
   rational operator*=( const rational & rhs ){
      return (counter = counter * rhs.counter, denominator *= rhs.denominator );
   }
